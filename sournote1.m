function sournote1(action,varargin)
% Detection of sour notes in random melodic patterns built from a predefined scale
% January 2021

persistent  poignee  nom_sujet  essai  total_essais ... 
                rep_juste  rep  Hits  False_alarms  Misses  Correct_rejections ...
                echelle Nnotes scale ...
                amp F_echan  F_ref  F  duree_note  duree_ISI;  

if nargin==2 % 2 arguments dans la fonction sournote1
    poignee = varargin{1}; % poignee est le 1er (et le seul) des varargin suivant "action"
end
if nargin==0 % on exécute sournote1 tout court
...
switch(lower(action))
    ...
    fichier_resultats = which('Résultats_sournote1.txt'); % donne le chemin du fichier Résultats_sournote1.txt
    ...
    fprintf(fid,'%s;%d;%d;%d;%s;%d;%d;%d;%d;%d\n', ...
        nom_sujet, echelle, SOAms, Frefround, moment, total_essais, Hits, Correct_rejections, Misses, False_alarms); 
    st = fclose(fid);
