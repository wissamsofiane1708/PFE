function sournote11(action,varargin)

% Similaire à sournote8 (et sournote10), sauf qu'on utilise ici les échelles
% [0 1 3 5 8 10] (phrygien dépourvu de 7 et 12) et
% [0 1 3 6 8 10] (locrien dépourvu de 5 et 12).
% L'absence (nécessaire) de la note 12 va être compensée par une répétition de la note 0.
% Septembre 2022
...
if nargin==2 % 2 arguments dans la fonction sournote11
    poignee = varargin{1}; % poignee est le 1er (et le seul) des varargin suivant "action"
end
if nargin==0 % on exécute sournote11 tout court
...
switch(lower(action))
    ...
    while (echelle < 0) || (echelle > 2)
            ...
            if echelle==0
                scale = [0 0 1 3 8 10]; % pour la familiarisation ; les éléments sont communs aux échelles 1 et 2
            elseif echelle==1
                scale = [0 0 1 3 5 8 10]; % échelle A    
            elseif echelle==2
                scale = [0 0 1 3 6 8 10]; % échelle B
            end
    end
    Nnotes = size(scale, 2);
    total_essais = input('nombre d''essais :  ');
    interface_sournote11;
    ...
    if echelle==0
        chance = ceil(3*rand);
        if chance==1
            scalediff = [0 0 2 3 8 10]; % 1 devient 2
        elseif chance==2
            scalediff = [0 0 1 3 8 9]; % 10 devient 9
        elseif chance==3
            scalediff = [0 0 1 3 8 11]; % 10 devient 11 
        end
    elseif echelle==1
        chance = ceil(4*rand);
        if chance==1
            scalediff = [0 0 2 3 5 8 10]; % 1 devient 2
        elseif chance==2
            scalediff = [0 0 1 3 6 8 10]; % 5 devient 6 ; on obtient l'échelle B
        elseif chance==3
            scalediff = [0 0 1 3 5 8 9]; % 10 devient 9 
        elseif chance==4
            scalediff = [0 0 1 3 5 8 11]; % 10 devient 11     
        end
    elseif echelle==2
        chance = ceil(4*rand);
        if chance==1
            scalediff = [0 0 2 3 6 8 10]; % 1 devient 2
        elseif chance==2
            scalediff = [0 0 1 3 5 8 10]; % 6 devient 5 ; on obtient l'échelle A
        elseif chance==3
            scalediff = [0 0 1 3 6 8 9]; % 10 devient 9
        elseif chance==4
            scalediff = [0 0 1 3 6 8 11]; % 10 devient 11
        end   
    end
    if rep_juste == 1
    ...
    case 'archivage',
    fileID = fopen('RésultatsLD_sournote11.txt','a');
