...
% Similaire à sournote8, sauf que :  
% 1) on utilise ici l'échelle majeure et l'échelle lydienne (mode de fa), pour opposer quarte juste et quarte augmentée ;
% 2) les modifications de notes sont de 3 types possibles (au lieu de 4) pour chaque échelle ;
% Septembre 2022
...
if nargin==0 % on exécute sournote10 tout court
...
switch(lower(action))
    ...
    while (echelle < 0) || (echelle > 2)
            ...
            if echelle==0
                scale = [0 4 7 9 12]; % pour la familiarisation ; les éléments sont communs aux échelles 1 et 2
            elseif echelle==1
    ...
    case 'archivage',
    fileID = fopen('RésultatsLD_sournote10.txt','a');
